...
class _Booking {
  ...
  String get displayTime {
    final start = DateFormat.jm().format(
      DateFormat(
        'HH:mm',
      ).parse('$startHour:${startMin.toString().padLeft(2, '0')}'),
    );
...
class _CourtAssignmentBoardState extends ConsumerState<CourtAssignmentBoard> {
  ...
  Future<void> _deleteReservation(_Booking booking) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(S.of(context).reservationDeleted)));
    }
  }
  ...
  void _showEditDialog(_Booking booking) {
    ...
    // Snapshot of currently loaded bookings — used to pre-filter which fields
    // are offered in the dropdown. The server re-checks on Update, so a stale
    // snapshot can never double-book.
    final allBookings =
        ref
            .read(
              boardBookingsProvider((
            ...
            .valueOrNull
            ?.map(_Booking.fromMap)
            .toList() ??
        [];
    final sizes =
        ref.read(courtSizesProvider(widget.courtId)).valueOrNull ?? {};
    final numFields = sizes[booking.size] ?? 1;

    // A field is offered when no other booking on it overlaps this booking's
    // exact time window (the current field is always offered).
    bool hasConflictOn(int field) {
      final start = booking.startHour * 60 + booking.startMin;
      final end = start + booking.duration * 60;
      return allBookings.any((b) {
        if (b.id == booking.id || b.fieldNumber != field) return false;
        final bStart = b.startHour * 60 + b.startMin;
        return bStart < end && start < bStart + b.duration * 60;
      });
    }

    final availableFields = [
      for (int f = 1; f <= numFields; f++)
        if (f == booking.fieldNumber || !hasConflictOn(f)) f,
    ];

    int selectedField = booking.fieldNumber;
    bool updating = false;

    Future<void> applyFieldChange(
      BuildContext ctx,
      ...
      try {
        result = await ReservationService.changeReservationField(
          reservationId: booking.id,
      ...
      if (result['success'] == true) {
        messenger.showSnackBar(SnackBar(content: Text(s.reservationUpdated)));
      } else if (result['reason'] == 'field_taken') {
        messenger.showSnackBar(
          SnackBar(
            content: Text(s.fieldTakenError),
            backgroundColor: Colors.orange.shade700,
          ),
        );
      } else {
  ...
  List<Widget> _buildEditActions(
    ...
    bool updating,
    void Function(void Function()) setDialogState,
    Future<void> Function(BuildContext, void Function(void Function()))
    applyFieldChange,
  ) {
    // A field change is pending → offer only Close + Update to keep it simple.
    if (selectedField != booking.fieldNumber) {
      return [
        TextButton(
          onPressed: updating ? null : () => Navigator.of(ctx).pop(),
          child: Text(s.close, style: TextStyle(color: Colors.grey.shade600)),
        ),
        ElevatedButton.icon(
          ...
          label: Text(s.update),
          onPressed:
              updating ? null : () => applyFieldChange(ctx, setDialogState),
        ),
      ];
    }
          ...
          final confirmed = await showDialog<bool>(
            ...
            builder:
                (_) => AlertDialog(
                  ...
                  content: Text(
                    s.deleteBookingConfirm(booking.name, booking.displayTime),
                  ),
                  actions: [
                    TextButton(
                      onPressed: () => Navigator.of(context).pop(false),
    ...
    final picked = await showDatePicker(
      ...
      builder:
          (ctx, child) => Theme(
            data: Theme.of(
              ctx,
            ).copyWith(colorScheme: ColorScheme.light(primary: ehjezGreen)),
            child: child!,
          ),
    );
    if (picked != null) {
  ...
  String _formatHour(int h) => DateFormat.j().format(DateTime(0, 1, 1, h % 24));
  ...
  Widget _bookingBlock(_Booking b) {
    ...
    return Positioned(
      ...
      child: GestureDetector(
        onTap: () => setState(() => _selectedBooking = isSelected ? null : b),
        ...
        Expanded(
          child: Row(
            ...
            children: List.generate(numFields, (fieldIndex) {
              ...
              final blkColor = _blockColors[fieldIndex % _blockColors.length];
              final fieldBookings =
                  bookings.where((b) => b.fieldNumber == fieldNum).toList();
          ...
          child: Row(
            children: [
              ...
              OutlinedButton.icon(
                ...
                label: Text(s.addBooking, style: const TextStyle(fontSize: 13)),
              ...
              AnimatedContainer(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(s.edit, style: const TextStyle(fontSize: 13)),
                  style: ElevatedButton.styleFrom(
                    backgroundColor:
                        _selectedBooking != null
                            ? ehjezGreen
                    ...
                    return GestureDetector(
                      ...
                      child: AnimatedContainer(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(20),
                          border: Border.all(
                            color: selected ? ehjezGreen : Colors.grey.shade300,
                          ),
                        ),
                        child: Text(
